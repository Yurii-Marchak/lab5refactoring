...
def test_add_new_book_success(book_service, book_repo_mock):
    ...
    dto = BookCreateDTO(title="1984", author="Orwell", isbn="111-222", copies=5)
...
def test_add_existing_book_updates_copies(book_service, book_repo_mock):
    existing_book = Book(id="book_1", title="1984", author="Orwell", isbn="111-222", available_copies=2)
    book_repo_mock.find_by_isbn.return_value = existing_book
    dto = BookCreateDTO(title="1984", author="Orwell", isbn="111-222", copies=3)
...
def test_search_books_found(book_service, book_repo_mock):
    mock_books = [
        Book(id="1", title="Python 101", author="A", isbn="123", available_copies=1),
        Book(id="2", title="Advanced Python", author="B", isbn="456", available_copies=2)
    ]
